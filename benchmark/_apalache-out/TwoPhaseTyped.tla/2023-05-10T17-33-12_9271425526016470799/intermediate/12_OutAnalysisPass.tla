...
*)
Init_si_0000 ==
  Skolem((\E t_3$1 \in [RM -> { "working", "prepared", "committed", "aborted" }]:
      rmState' := t_3$1))
    /\ Skolem((\E t_4$1 \in { "init", "committed", "aborted" }:
      tmState' := t_4$1))
    /\ Skolem((\E t_5$1 \in SUBSET RM: tmPrepared' := t_5$1))
    /\ Skolem((\E t_6$1 \in SUBSET ({
      [type |-> t$1, rm |-> r$1]:
        t$1 \in {"Prepared"},
        r$1 \in RM
    }
