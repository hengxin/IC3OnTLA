...
(*
  @type: (() => Bool);
*)
Init_si_0000 == rmState' := [ rm$1 \in RM |-> "working" ]
...
Next_si_0001 ==
  \E rm$3 \in RM:
    rmState[rm$3] = "prepared"
      /\ (\A rm$4 \in RM: rmState[rm$4] \in { "prepared", "committed" })
      /\ rmState' := [ rmState EXCEPT ![rm$3] = "committed" ]
...
Next_si_0002 ==
  \E rm$5 \in RM:
    rmState[rm$5] \in { "working", "prepared" }
      /\ (\A rm$6 \in RM: ~(rmState[rm$6] = "committed"))
      /\ rmState' := [ rmState EXCEPT ![rm$5] = "aborted" ]
